...
function randomDamage(weapon) {
	//* Calculate a damage value between a weapons minimum and maximum values (both inclusive)
	return Math.floor(
		Math.random() * (weapon.maxDamage - weapon.minDamage + 1) +
			weapon.minDamage
	);
}

function attackSuccess(weapon) {
	//* Calculate success of an attack given a weapon's accuracy
...
const playerHealthLabel = document.getElementsByClassName(
	'player-health-label'
)[0];
const computerHealthBar = document.getElementsByClassName(
	'computer-health-bar'
)[0];
const computerHealthLabel = document.getElementsByClassName(
	'computer-health-label'
)[0];

function updateHealthBars() {
	//* Update styling of health bars
...
for (const player of GAME.players) {
	//* Give each player an attack() function that takes a target. The 'target' argument
	//* might be useful if more enemies were added in the future.
	player.attack = function (target) {
		...
		document
			.getElementsByClassName('announcement-label')[0]
			.classList.remove('update'); //* Necessary for announcement label transitions

		setTimeout(function () {
			if (player.currentWeapon == null) {
				//* Check if the player has equipped a weapon

				document.getElementsByClassName(
					'announcement-label'
				)[0].innerHTML = 'Equip a weapon first!';
				return;
				//* Return from the function without attacking
			} else if (attackSuccess(player.currentWeapon)) {
				let damage = randomDamage(player.currentWeapon);

				//* If the attack is successful given the equipped weapon's accuracy,
				//* a damage value between the minimum damage and maximum damage (both inclusive)
				//* is calculated and subtracted from the target's health

				target.health -= damage;

				document.getElementsByClassName(
					'announcement-label'
				)[0].innerHTML = `Hit for ${damage} damage points!`;
...
function coinToss() {
	//* Give each player an equal chance to attack first
...
function switchTurn(currentTurn) {
	//* Swap who is currently attacking
...
function humanPlayerSelectWeapon(button) {
	if (humanPlayerSelectedButton !== null) {
		//* If a button has already been selected, play the animation in reverse
...
function computerEquipRandomWeapon() {
	...
	weaponButtons.forEach((button) => {
		//* Reset the computer's buttons before equipping a new weapon
	...
	setTimeout(() => {
		...
		const selectedButton =
			weaponButtons[Math.floor(Math.random() * weaponButtons.length)];
...
function checkGameOver() {
	if (humanPlayer.health <= 2) {
		//* The black health label text is difficult to see when it falls outside of the health bar.
		...
		if (computer.health <= 0) {
			computer.health = 0;
			GAME.gameOver = true;
			document.getElementsByClassName('player-name-label')[0].innerHTML =
				'Winner!';
			document.getElementsByClassName(
				'player-name-label'
			)[0].style.color = 'black';
...
function debounce(func, wait) {
	//* Sometimes player.attack() would be called multiple times
	//* in a sinle turn. This debounce function prevents a function
	//* from being called twice in a given time frame.
	//! This wasn't actually the solution to the issue,
	//! but it's cool and maybe useful elsewhere so I'm leaving it here.
	let timeout;
	return function () {
...
function gameLoop() {
	if (humanPlayer.health <= 0 || computer.health <= 0) {
		//* Exit the loop when one of the players is dead
		GAME.gameOver = true;
		return;
	}
	if (GAME.currentTurn === humanPlayer) {
		if (humanPlayer.currentWeapon == null) {
			//* Check if the player has equipped a weapon
...
function attachAttackButtonEventListener() {
	document
		.getElementsByClassName('attack-button')[0]
...
function startGame() {
	...
	document.getElementsByClassName('combat-grid')[0].innerHTML =
		'<label class="announcement-label"></label><button class="attack-button" type="button">Attack</button>';
	coinToss(); //* Randomly select the first attacker

	document.getElementsByClassName('announcement-label')[0].innerHTML =
		GAME.currentTurn.name + ' is attacking first';
	setTimeout(function () {
		document
			.getElementsByClassName('announcement-label')[0]
			.classList.add('update');
...
let targetSword = document.getElementsByClassName(
	'player-weapon-sword-button'
)[0];
...
targetAxe.addEventListener('mouseover', () => {
	//* show circle when mouse is hovering over the button
...
targetAxe.addEventListener('mouseout', () => {
	//* Hide circle when mouse is no longer hovering over the button
